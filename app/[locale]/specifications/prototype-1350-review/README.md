...
responsibility easiest to scan. No design is accepted yet. Review the options
before implementing a production change.
...
| --- | --- | --- |
| `variant=baseline` | Eight-column reference | Recreates the existing presentation with safe controls; it is not a full production-page clone. |
...
behavior. Search and expanded areas survive variant switches; a full reload
...
Names, counts, create, edit, delete and co-author controls show a local preview
...
tillstånd** for the current variant, data source, filter, expanded rows,
...
| --- | --- | --- | --- |
| More space for names | A | Compare 0 and A at 1440 × 900 with navigation expanded. Read the fourth example. | Names receive the remaining table width; classifications occupy one column instead of three. |
| Alternative information hierarchy | B | Read the fourth example and its second line. | Name, responsible person, count and actions lead; labeled classifications and areas appear below. |
...
| Stable responsibility block | A/B/C | Inspect the first four examples, especially the long personal name. | Person name appears above a smaller HSA-id. Long names can wrap independently. |
| HSA-id wrapping | A/B/C | Compare Karl Persson with the baseline; inspect the exceptionally long fourth HSA-id. | Normal identifiers stay on one line. An unusually long identifier remains accessible through local horizontal scrolling. |
| Missing and anonymized names | All | Inspect examples five and six; also open `/en/specifications?variant=A`. | Missing names retain the HSA-id. The sentinel is displayed as Anonym/Anonymous. |
...
| Stable number placement | A/B/C | Compare 0, 3, 128 and 1200. Click a nonzero count. | A right-aligns tabular digits. B/C label the count. Clicking updates the preview message. |
...
| Area expansion retained | All | Expand the fourth row with click, then focus its toggle and press Enter. Switch variants while expanded. | All six areas remain available. Expanded state survives the switch. |
...
| Safe previews | All | Click create, name, count, co-authors, edit and delete. | Only the local preview message changes; no database mutation request is sent. |
...
| Shareable variant | All | Copy the URL, reload and compare the selected button. | The variant is retained in `?variant=`; transient filter/expansion state resets. |
...
| State inspection | All | Open Visa tillstånd and interact with search, areas and actions. | The displayed JSON reflects the current review state. |
...
3. Read the longest name and person name. Expand its six requirement areas.
...
results, keyboard switching, expansion retained across variants, action previews,
English identity fallbacks and URL reload behavior are exercised.

At 1440 pixels with expanded navigation, example row heights are 69–130 pixels
in the baseline reference, 72–79 in A, 110–111 in B, and 261–270 per card in C.
A gives the longest name roughly 304 pixels of text width versus 144 in the
reference. This supports trying A first for long names; it does not establish
a user-tested winner or make every short row smaller.
...
preview controls and examples. It shares prototype expansion and empty action
slots, so it is not pixel-identical to the current production list. The original
...
| `prototype-1350.data.ts` | Seven in-memory examples covering edge cases. |
