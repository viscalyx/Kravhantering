...
export async function GET(
  ...
  try {
    return NextResponse.json(
      await getSpecificationLocalDeviation(db, deviationId),
    )
  } catch (error) {
...
export async function PUT(
  ...
  try {
    await updateSpecificationLocalDeviation(db, deviationId, {
...
export async function DELETE(
  ...
  try {
    await deleteSpecificationLocalDeviation(db, deviationId)
    return NextResponse.json({ ok: true })
  } catch (error) {
    if (isRequirementsServiceError(error)) {
      return NextResponse.json(
