...
test('a promoted generation contains only each role allowlist with restrictive modes', async () => {
...
test('verification and inspection return safe certificate metadata only', async () => {
...
test('ensure reuses a valid generation without replacing material', async () => {
...
test('persistent ensure renews at 30 days while ephemeral runs stay fresh-per-run', () => {
...
test('per-domain rotation replaces only that CA and its two leaves', async () => {
  const beforeInspect = await inspectGeneration({ profile, rootDir: testRoot })
  ...
  for (const name of Object.keys(profile.trustDomains)) {
    ...
    if (name === 'kong-to-adapter') {
      assert.notEqual(afterDomain.ca.digestSha256, beforeDomain.ca.digestSha256)
      assert.notEqual(
...
test('rollback restores the prior selection and removes the failed generation', async () => {
...
test('finalization deletes a verified prior generation only after promotion', async () => {
  const rotated = await rotateTrustDomain({
    issuerRoot,
    lifetime: 'ephemeral',
    profile,
...
test('verification fails closed for bundle drift, mismatched keys, wrong trust, and permissions', async t => {
