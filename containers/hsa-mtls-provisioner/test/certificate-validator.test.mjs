...
import { validateCertificateMaterial } from '../src/provisioner.mjs'
...
test('the leaf validator accepts the exact trusted single-purpose client', async () => {
...
test('the leaf validator rejects a trusted client with the wrong exact subject', async () => {
...
test('the leaf validator rejects dual EKU, wrong key usage, and SHA-1 signatures', async t => {
  const fixtures = [
    {
...
test('the leaf validator rejects not-yet-valid and expired material at the evaluation time', async () => {
  const material = await issueClient({ name: 'validity-client' })
  const certificate = new X509Certificate(await readFile(material.certificate))
  const options = {
    caPath: caCertificate,
    certificatePath: material.certificate,
    keyPath: material.key,
...
test('the leaf validator rejects a CA leaf and malformed PEM', async () => {
