...
/var/lib/kravhantering/hsa-mtls/
  generations/<generation-id>/bundles/{app,kong,adapter,mock}/
  staged/<generation-id>/bundles/{app,kong,adapter,mock}/
  selection.json
```

No deployed topology selects these bundles until the strict-mTLS atomic
cutover. Runtime orchestration will mount only the selected role directory,
read-only, at `/run/kravhantering/hsa-mtls`.
...
- `rotate <trust-domain>` stages and promotes a replacement CA and both leaves
  for exactly one trust domain.
