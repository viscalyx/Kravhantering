...
interface PackageLocalRequirementDetailClientProps {
...
export default function PackageLocalRequirementDetailClient({
  localRequirementId,
  needsReferences,
  onChange,
  packageSlug,
}: PackageLocalRequirementDetailClientProps) {
  ...
  const [requirement, setRequirement] =
    useState<PackageLocalRequirementDetail | null>(null)
  ...
  const fetchRequirement = useCallback(async () => {
    ...
    try {
      ...
      setRequirement((await response.json()) as PackageLocalRequirementDetail)
    } catch (fetchError) {
      setRequirement(null)
      setError(
  ...
  const handlePrint = useCallback(() => {
    ...
    window.open(
      `/${locale}/requirement-packages/${encodeURIComponent(
        packageSlug,
      )}/reports/print/list?refs=${encodeURIComponent(requirement.itemRef)}`,
      '_blank',
    )
  }, [locale, packageSlug, requirement?.itemRef])
  ...
  const handleEditSubmit = useCallback(
    ...
    [localRequirementId, packageSlug, refreshAll, tc],
  )
  ...
  const handleEditDeviation = useCallback(
    async (motivation: string, createdBy: string) => {
      ...
      await performDeviationMutation(
        `/api/package-local-deviations/${latestDeviation.id}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleRequestReview = useCallback(async () => {
    ...
    await performDeviationMutation(
      `/api/package-local-deviations/${latestDeviation.id}/request-review`,
      { method: 'POST' },
      td('reviewFailed'),
    )
  }, [latestDeviation, performDeviationMutation, td])
  ...
  const handleRevertToDraft = useCallback(
    async (event?: React.MouseEvent<HTMLButtonElement>) => {
      ...
      await performDeviationMutation(
        `/api/package-local-deviations/${latestDeviation.id}/revert-to-draft`,
        { method: 'POST' },
        td('revertFailed'),
      )
    },
  ...
  const handleRecordDecision = useCallback(
    async (decision: 1 | 2, motivation: string, decidedBy: string) => {
      ...
      await performDeviationMutation(
        `/api/package-local-deviations/${latestDeviation.id}/decision`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const metadata = [
    ...
    },
    {
      ...
      value:
        requirement.packageItemStatusNameEn ||
        requirement.packageItemStatusNameSv ? (
          <span className="inline-flex items-center gap-2">
            {requirement.packageItemStatusColor ? (
              <span
                className="h-2.5 w-2.5 rounded-full"
                style={{ backgroundColor: requirement.packageItemStatusColor }}
              />
            ) : null}
            {locale === 'sv'
              ? (requirement.packageItemStatusNameSv ??
  ...
  const detailContext =
    'requirement package detail > detail pane: package local requirement'
  ...
  const references = requirement.normReferences.map(reference => ({
    href: reference.uri,
    id: `package-local-normref-${reference.id}`,
  ...
  const scenarios = requirement.scenarios.map(scenario => ({
    id: `package-local-scenario-${scenario.id}`,
    label:
      locale === 'sv'
  ...
  const canMutateLocalRequirement =
    requirement.packageItemStatusId === INCLUDED_PACKAGE_ITEM_STATUS_ID &&
    !hasPendingDeviation
  ...
  return (
    <div
      {...devMarker({
        ...
        name: 'detail pane',
        priority: 330,
        value: 'package local requirement',
      })}
    >
      {showEditForm ? (
