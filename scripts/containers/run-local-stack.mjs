...
function runNginx(config, options = {}) {
  ...
  runCommand(
    ...
    [
      ...
      '--volume',
      `${path.resolve(cwd, 'containers/production/nginx/templates/readiness-locations.conf')}:/etc/nginx/snippets/readiness-locations.conf:ro`,
      '--volume',
      `${path.resolve(cwd, 'containers/nginx/readiness-probes.conf')}:/etc/nginx/snippets/readiness-probes.conf:ro`,
      '--volume',
