...
const base = `http://localhost:${process.env.PROTOTYPE_PORT ?? 3135}`
const output = 'tmp/prototype-1355'
    ...
    for (const theme of ['light', 'dark']) {
      for (const navigation of ['collapsed', 'expanded']) {
        ...
        for (const variant of ['original', 'A', 'B', 'C']) {
          ...
          await page.locator('[data-question-id]').first().waitFor()
          await page.waitForFunction(
            ({ variant, theme }) =>
              document
                .querySelector('[data-prototype-variant]')
                ?.getAttribute('data-prototype-variant') === variant &&
