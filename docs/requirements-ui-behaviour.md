...
- On `xl` and wider package-detail layouts, the `Krav i underlaget` and
...
- Client-side filtering in the package detail matches on `specificationItemStatusId`.
...
4. **Package count** — read-only count of how many requirements specifications
...
- In the package-detail left panel, specification-local rows are visually
  marked with a dedicated icon marker so they can be distinguished from
  library requirements pinned into the package.
...
- Specification-local rows use short package-scoped Krav-ID values such as
...
- The specification-local inline detail pane now reuses the same core content-card
  ...
  scenarios.
- When a library requirement is opened from the package list `Krav i kravunderlag`,
...
- Specification-local inline detail now follows the package-item detail chrome more
...
- Each value is a package-context item reference:
  - `lib:<specificationItemId>` for a library requirement in the package
