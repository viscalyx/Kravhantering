...
This guide helps developers and integration operators configure, verify, and
maintain the server-side person lookup used for responsibility assignments.
Browser sign-in, MCP Bearer-token
...
leads and co-authors. The browser never calls Kong or the HSA directory
directly. It only talks to the app's protected same-origin routes.
...
contacting HSA lookup, HSA OAuth discovery, token, or SOAP services. The app
loads and validates its transport configuration once, when readiness or the
first lookup needs the process-wide snapshot; the adapter loads its snapshot
at startup. Configuration and certificate changes require process recreation.

`HSA_PERSON_LOOKUP_TIMEOUT_MS` controls the app-side timeout for each outbound
request. It defaults to 5000 milliseconds and accepts integers from 1 to 30000.
Keep the default unless the approved integration path requires a different
timeout.
...
read-only mounts and the server name is one exact DNS identity. If the URL and
all HSA authentication settings are absent, the live capability is unavailable
without failing the HSA readiness check, and local `Kravansvarsperson` reuse
continues.
Leaving TLS or OAuth settings configured without the URL fails readiness. Set
...
`HSA_PERSON_LOOKUP_OAUTH_CA_PATH` is set, snapshot loading validates a
current CA bundle with exact CA key usage and at least one self-signed root.
Any intermediate CA must have a complete path to a root in that bundle. No
certificate in the OAuth CA bundle may also appear in the lookup CA bundle,
even if stored at a different path.

The client certificate file contains the leaf followed by any presented PEM
intermediates. Snapshot loading builds one bounded, cryptographically verified
path through every presented intermediate to a self-signed root in the CA
bundle.
...
Discovery, token, and REST lookup responses are each limited to 64 KiB. The
adapter accepts `text/xml` and `application/soap+xml` responses up to 1 MiB.
Media type and streaming byte limits apply to successful and error responses,
and an over-limit stream is
...
session, CSRF, purpose, and scope before reading a local person or calling
HSA. Scope authorization can itself require database reads.

The HSA verification quota is stored in SQL Server and shared across app
instances. Each 60-second window allows 50 requests per caller, 10 per target,
and 10 per caller-target pair. Keys use non-reversible HMAC fingerprints;
target fingerprints normalize HSA-id case to prevent variant-based bypasses.
A limited request returns code `hsa_verification_throttled`, HTTP `429`,
`Retry-After`, and `retryAfterSeconds` before reading a local person or calling
HSA. If the quota cannot be enforced, the route returns HTTP `503` with code
`service_unavailable`, `Retry-After: 5`, and `retryAfterSeconds: 5`, also
without performing either lookup. Both verification modes require this quota
check.
...
A successful response also contains signed `evidence`, valid for five minutes,
and its `expiresAt` time. The evidence is bound to the authenticated caller,
normalized target HSA-id, assignment purpose, optional scope ID, and verified
person record. Final assignment routes reject missing, expired, tampered, cross-actor,
cross-target, cross-purpose, or cross-scope evidence. Verification outcomes
(`success`, `not_found`, `conflict`, `throttled`, `quota_unavailable`, and
`provider_failure`) are audited with target fingerprints and operational
metadata only, never the raw target HSA-id or returned person details.
...
require OAuth2 client credentials before accepting the same REST request. Use
the [REST facade OpenAPI contract](../../openapi/hsa-person-lookup.yaml) when
implementing or validating the integration endpoint.
...
DB-less and strict. Kong exposes only HTTPS
`POST /hsa/person-records/lookup` and routes that request to
`hsa-person-lookup-adapter`. The adapter owns the
...
sequenceDiagram
    ...
    alt Field leaves focus
        ...
        Verify->>DB: Enforce shared HSA verification quota
...
evidence. Adding an identity through these editors requires evidence, even
when it comes from local reuse. Initial specification and package creation
instead derive the responsible person or lead from the authenticated actor.
...
See the [provisioner lifecycle commands](../../containers/hsa-mtls-provisioner/README.md#lifecycle-commands)
for generation storage and command semantics. These test-PKI commands apply to
repository-owned topologies; production deployment owners manage their own
