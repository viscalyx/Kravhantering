...
const translations: Record<string, string> = {
  ...
  'package.deleteLocalRequirementConfirm': 'Delete this requirement?',
  'package.deleteLocalRequirementConfirmTitle': 'Delete local requirement',
  'package.localRequirementActionDisabledTooltip':
    'This local requirement can only be edited or removed when Usage status is Included and no deviation is pending.',
  'package.editLocalRequirement': 'Edit local requirement',
  'package.localRequirementNotFound': 'Local requirement not found',
  ...
  'requirement.packageItemStatus': 'Package item status',
  'requirement.packageLocalBadge': 'Unique',
  'requirement.packageLocalTooltip': 'This row is a package-local requirement.',
...
vi.mock('@/components/PackageLocalRequirementForm', () => ({
  default: () => <div data-testid="package-local-form" />,
}))
...
describe('PackageLocalRequirementDetailClient', () => {
  beforeEach(() => {
  ...
  it('reuses the shared detail card layout and catalog-style action column', async () => {
    vi.mocked(fetch)
      .mockImplementationOnce(() =>
        okJson({
          acceptanceCriteria: 'Package local acceptance',
    ...
    expect(
      await screen.findByText('Package local description'),
    ).toBeInTheDocument()
    expect(screen.getByText('Acceptance criteria')).toBeInTheDocument()
    expect(screen.getByText('Package local acceptance')).toBeInTheDocument()
    ...
    const inlineInset = screen
      .getByText('Package local description')
