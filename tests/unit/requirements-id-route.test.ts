...
describe('requirements/[id] route', () => {
  ...
  describe('DELETE', () => {
    ...
    it.each([
      ...
      'archives requirement with fallback response when no detail is returned for %s',
      async (_label, ref, expectedId, expectedUniqueId) => {
        mockManageRequirement.mockResolvedValue({})

        const req = new NextRequest(
          `http://localhost/api/requirements/${ref}`,
          {
            method: 'DELETE',
          },
        )
