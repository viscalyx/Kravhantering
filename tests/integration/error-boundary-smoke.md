...
- The gated pages render a tiny client trigger that throws after hydration, so
  the App Router boundary is exercised without reporting an expected server
  render error from the dev server.
...
sequenceDiagram
    ...
    participant C as ClientTrigger
    participant E as ErrorBoundary

    P->>R: Open /sv/error-boundary-test
    R-->>C: Render gated trigger
    C-->>E: Throw test render error after hydration
...
## shows English admin recovery for an admin client render failure

### Purpose: Admin Recovery

Verifies that an admin client render failure under `/en` renders English
recovery copy, hides the raw thrown message, exposes a retry button, and
prioritizes the admin overview as the safe destination.
...
sequenceDiagram
    ...
    R-->>C: Render gated trigger
    C-->>E: Throw test admin render error after hydration
