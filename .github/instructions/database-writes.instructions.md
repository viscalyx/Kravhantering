...
- Use the active DB client's supported atomic primitive, such as `transaction()` or Cloudflare D1 `batch()`.
- Do not emit raw SQL `BEGIN`, `COMMIT`, `ROLLBACK`, or `SAVEPOINT` for Cloudflare D1 paths.
