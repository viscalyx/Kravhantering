...
- Use the active DB client's supported atomic primitive, such as `transaction()` on the current SQLite client or `DataSource.transaction(...)` / the TypeORM transaction manager on the SQL Server path.
