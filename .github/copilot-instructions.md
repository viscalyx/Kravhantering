...
- When adding or changing outward-facing lifecycle, specification-item status, MCP tool, report column, or admin-default behavior, update `tests/quality/QUALITY.md` and add a matching scenario in `tests/quality/functional.test.ts`. See `.github/instructions/quality-spec.instructions.md`.
