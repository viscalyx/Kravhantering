...
function buildSortDescriptor(opts) {
  ...
  const textOrder = col => ({
    ...
    kind: 'text',
    nullRankExpression: `CASE WHEN ${col} IS NULL OR LTRIM(RTRIM(${col})) = '' THEN 1 ELSE 0 END`,
    orderBy: buildTextOrderExpression(col, dir, 'requirement.unique_id ASC'),
  })
  const numberOrder = col => ({
    expression: col,
    kind: 'number',
    nullRankExpression: `CASE WHEN ${col} IS NULL THEN 1 ELSE 0 END`,
    orderBy: `CASE WHEN ${col} IS NULL THEN 1 ELSE 0 END ASC, ${col} ${dir}, requirement.unique_id ASC`,
  })
  ...
  switch (sortBy) {
    ...
    default:
      return {
        ...
        orderBy: `requirement.unique_id ${dir}`,
      }
  }
}
...
function buildSeekCondition(opts, builder, descriptor) {
  ...
  if (descriptor.kind === 'unique') {
    const anchorValue = builder.push(anchor.sortValue)
    return `${value} ${dir === 'desc' ? '<' : '>'} ${anchorValue}`
  }

  const anchorRank = builder.push(anchor.nullRank)
  const anchorValue = builder.push(anchor.sortValue)
  const anchorUniqueId = builder.push(anchor.uniqueId)

  let valueAfter
  if (anchor.sortValue == null) {
    valueAfter =
      dir === 'asc'
        ? `(${value} IS NOT NULL OR (${value} IS NULL AND requirement.unique_id > ${anchorUniqueId}))`
        : `(${value} IS NULL AND requirement.unique_id > ${anchorUniqueId})`
  } else {
    const comparison = dir === 'desc' ? '<' : '>'
    const nullAfter = dir === 'desc' ? ` OR ${value} IS NULL` : ''
    valueAfter = `((${value} IS NOT NULL AND ${value} ${comparison} ${anchorValue}) OR (${value} = ${anchorValue} AND requirement.unique_id > ${anchorUniqueId})${nullAfter})`
  }

  return `(${rank} > ${anchorRank} OR (${rank} = ${anchorRank} AND ${valueAfter}))`
}
...
export function buildRequirementListSql(opts = {}) {
  ...
  let pagination = ''
  if (opts.limit != null) {
    pagination = ` OFFSET 0 ROWS FETCH NEXT ${builder.push(opts.limit)} ROWS ONLY`
  }
...
export function buildRequirementListAnchorSql(opts = {}, anchorId) {
  const builder = createSqlBuilder()
  const conditions = buildListConditions(opts, builder)
  conditions.push(`requirement.id = ${builder.push(anchorId)}`)
  const whereClause = `WHERE ${conditions.join(' AND ')}`
  const descriptor = buildSortDescriptor(opts)
  ...
  return {
    ...
    SELECT TOP (1)
      requirement.unique_id AS uniqueId,
      ${descriptor.nullRankExpression} AS nullRank,
      ${descriptor.expression} AS sortValue
    ${LIST_FROM_CLAUSE}
    ${whereClause}
  `,
  }
