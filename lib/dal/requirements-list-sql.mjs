...
export const REQUIREMENT_LIST_TEXT_SORT_KEY_LENGTH = 48

function boundedTextExpression(column) {
  return `LEFT(${normalizedTextExpression(column)}, ${REQUIREMENT_LIST_TEXT_SORT_KEY_LENGTH})`
}
...
function buildSortDescriptor(opts) {
  ...
  const textOrder = col => ({
    expression: boundedTextExpression(col),
    kind: 'text',
    nullRankExpression: `CASE WHEN ${normalizedTextExpression(col)} IS NULL THEN 1 ELSE 0 END`,
    orderBy: `CASE WHEN ${normalizedTextExpression(col)} IS NULL THEN 1 ELSE 0 END ASC, ${boundedTextExpression(col)} ${dir}, requirement.id ASC`,
  })
...
function buildSeekCondition(opts, builder, descriptor) {
  ...
  if (descriptor.kind === 'unique') {
    ...
    return `${value} ${comparison} ${anchorValue}`
  }
  ...
  const comparison = dir === 'desc' ? '<' : '>'
  const equalValue = `(${value} = ${anchorValue} OR (${value} IS NULL AND ${anchorValue} IS NULL))`

  return `(${rank} > ${anchorRank} OR (${rank} = ${anchorRank} AND (${value} ${comparison} ${anchorValue} OR (${equalValue} AND requirement.id > ${anchorRequirementId}))))`
}
...
export function buildRequirementListSql(opts = {}) {
  ...
  return {
    ...
    SELECT ${top}
      ...
      ${sortDescriptor.nullRankExpression} AS cursorNullRank,
      ${sortDescriptor.expression} AS cursorSortValue,
      ${searchMatchColumns}
